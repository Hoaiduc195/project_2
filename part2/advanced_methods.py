...
from part1.ridge_lasso import compute_mae, compute_rmse, compute_r2
import pandas as pd
import numpy as np
...
class KernelRidgeRegression:
    ...
    def _linear_kernel(self, X1, X2):
        return matmul(X1, transpose(X2))

    def _polynomial_kernel(self, X1, X2):
        return (matmul(X1, transpose(X2)) + self.coef0) ** self.degree
...
class BayesianLinearRegression:
    ...
    def fit(self, X, y):
        ...
        beta_noise = 1.0 / self.sigma2

        SN_inv = self.alpha * I + beta_noise * (X.T @ X)

        self.posterior_cov = np.linalg.pinv(SN_inv)

        # Posterior mean:
        # m_N = beta S_N X^T y
        self.posterior_mean = beta_noise * (self.posterior_cov @ X.T @ y)

        self.posterior_mean = np.asarray(self.posterior_mean).reshape(-1)
    ...
    def predict(self, X, return_std=False):
        ...
        if self.posterior_mean is None:
            raise RuntimeError("BayesianLinearRegression must be fitted before prediction.")
        ...
        # var[y_*] = sigma2 + x_*^T S_N x_*
        variance = self.sigma2 + np.sum((X @ self.posterior_cov) * X, axis=1)

        variance = np.maximum(variance, 1e-12)
        std = np.sqrt(variance).reshape(-1)
